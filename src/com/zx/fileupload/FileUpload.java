...
public class FileUpload {
	...
	public void setFileUploadRequestTimeOut(int fileUploadRequestTimeOut) {
		if (isRunning)
			this.fileUploadProp
					.setFileUploadRequestTimeOut(fileUploadRequestTimeOut);
	}

	/**
	 * constructer Method .Generate a New FuleUploadManagerObject
	 * 
	 * @param fileUploadRequestTimeOut
	 *            fileUploadobject timeout(second),when a part uploaded then
	 *            reset time counter
	 * @param recycleCycle
	 *            the cycle of check fileUploadObject timeout (second)
	 */
	 ...
	 */
	public FileUploadBucketProp addUploadBucket(String bucketName,
			FileUploadBucketProp prop) {
	 ...
	 * @return
	 */
	public String addFileUploadRequest(String fileName, String bucketName,
			long length, Map<String, String[]> parameperMap) {

		FileUploadBucketProp bucketProp = fileUploadProp.getFileUploadPropMap()
				.get(bucketName);
		...
		boolean needCheck = bucketProp.needMd5Checking();
		synchronized (fileUploadObjectMap) {
			FileUploadObject fileUploadObject = fileUploadObjectMap.get(propId);
			if (fileUploadObject == null) {
				fileUploadObject = new FileUploadObject(bucketName, fileName,
						length, bucketProp);
				fileUploadObjectMap.put(propId, fileUploadObject);
			} else {
				needCheck = StringUtil.isNullString(fileUploadObject.getMd5Checking())
						 ||fileUploadObject.getMd5Checking().length() != 32;
			}
		}
		generateUploadPart(fileUploadObjectMap.get(propId));
		// return needParts
		return getNeedPartsString(propId,
				bucketProp.getMaxTransportThreadNum(), needCheck);
	}
	 ...
	 * @return
	 */
	public String transmitUploadPart(String ObjectId, int sequence,
			InputStream in) {
		...
		try {
			if (partConfig == null
					|| partConfig.getTransportStatue() != TransportStatue.WAITTING)
			...
			synchronized (partConfig) {
				...
				FilePartObject partObject = new FilePartObject(ObjectId,
						sequence, partConfig.getStartIndex(),
						partConfig.getLength(), in);
				if (uploadObject.getObjectProp().onFilePartUpload(partObject)) {
					...
					filePartMap.get(sequence).setTransportStatue(
							TransportStatue.INLINE);
				}
			}
		...
		return getNeedPartsString(ObjectId, 1, false);
	}

	public String cencelUpload(String objectId) {
		FileUploadObject uploadObject = fileUploadObjectMap.remove(objectId);
		if (uploadObject != null)
			uploadObject.getObjectProp().onFileUploadCenceled(objectId,
					uploadObject);
		else
			return UploadConstent.ERROR_CLIENT_NOREQUEST;
		return StringUtil.replaceString(UploadConstent.REQUEST_SUCCESS,
				objectId);
	}
	...
	public String setMD5(String id, String md5) {
		...
		if (uploadObject != null && md5.length() == 32)
			uploadObject.setMd5Checking(md5);
		else
			return UploadConstent.ERROR_CLIENT_NOREQUEST;
		return StringUtil.replaceString(UploadConstent.REQUEST_SUCCESS, id);
	}
	...
	public void start() {
		if (isRunning)
			return;
		onStart();
		this.uploaderManageThread = new UploaderManageThread(
				fileUploadObjectMap, fileUploadProp.getRecycleCycle(),
				fileUploadProp.getFileUploadRequestTimeOut());
	...
	private void generateUploadPart(FileUploadObject fileConfig) {
		if (fileConfig.getParts().isEmpty()
				&& fileConfig.getReceivedSize() == 0) {
			int partSize = fileConfig.getObjectProp().getPartSize();
			long i = 0;
			int sequence = 1;
			while (true) {
				if ((i + partSize > fileConfig.getFileSize())) {
					FilePartConfig filePargConfig = new FilePartConfig(i,
							(int) (fileConfig.getFileSize() % partSize));
					fileConfig.getParts().put(sequence, filePargConfig);
					break;
				}
				fileConfig.getParts().put(sequence,
						new FilePartConfig(i, partSize));
				i = i + partSize;
				sequence++;
			}
	...
	private String getNeedPartsString(String objectId, int partNum,
		...
		StringBuilder returnString = new StringBuilder(
				"{\"code\":0,\"value\":\"success\",\"id\":\"" + objectId
						+ "\",\"needMd5\":" + needMd5 + ",\"received\":"
						+ fileObject.getReceivedSize());
		returnString
				.append(doGetPartsString(objectId, fileObject, partNum) + '}');
	...
	private String doGetPartsString(String objectId,
		...
		synchronized (fileConfig.getParts()) {
			...
			Collection<Integer> partSeqs = ((HashMap<Integer, FilePartConfig>) fileConfig
					.getParts().clone()).keySet();
			if (partSeqs.isEmpty()) {
				fileConfig.getObjectProp().onFileUploadFinished(objectId,
						fileConfig);
				returnString
						.append("{\"partSeq\":-1,\"startIndex\":0 ,\"length\":0}");
			} else {
				int i = 0;
				for (Integer partSeq : partSeqs) {
					FilePartConfig filePartConfig = fileConfig.getParts().get(
							partSeq);
					...
					case WAITTING:
						if (new Date().getTime()
								- filePartConfig.getTransportTime() > fileConfig
								.getObjectProp().geTransportPartTimeOut() * 1000) {
							if (filePartConfig.getTransportStatue() == TransportStatue.WAITTING) {
								...
								fileConfig.getParts().put(++sequence,
										filePartConfig);
								fileConfig.setSequence(sequence);
								generateNeedPart(returnString, sequence,
										filePartConfig);
								i++;
							}
						}
	...
	private void generateNeedPart(StringBuilder returnString, int partSeq,
		...
		returnString.append("{\"startIndex\":" + filePartConfig.getStartIndex()
				+ ",\"length\":" + filePartConfig.getLength() + ",\"partSeq\":"
				+ partSeq + "},");
